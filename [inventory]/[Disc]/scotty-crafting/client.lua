...
function CraftItem(cate_id, item_id, amount)
	...
	or Config["craft_cost"] ~= nil and Config["craft_cost"] > 0 then
		...
		if type(cost) ~= "number" then
			SendNUIMessage({
				type = 'dialog',
				title = "Error de archivo de configuración",
				message = "Póngase en contacto con el administrador del sistema para solucionar el problema."
			})
			return
		end
...
Citizen.CreateThread(function()
	while true do
		...
		for i = 1, #modelFirePit do
			closestFirePit = GetClosestObjectOfType(myCoords.x, myCoords.y, myCoords.z, 2.5, modelFirePit[i], false, false)
			if closestFirePit ~= nil and closestFirePit ~= 0 then
				coords = GetEntityCoords(closestFirePit)
				local distance = GetDistanceBetweenCoords(myCoords.x, myCoords.y, myCoords.z, coords.x,coords.y,coords.z, true)
				if distance < 2.5 and fire < 1 then
					DisplayHelpText("~INPUT_CONTEXT~ para encender hoguera")
					if i >= 2 or i <= 2  then
						--TriggerEvent('vrp_frio:verificarIsqueiro',true,coords.x,coords.y,coords.z-0.7,modelFirePit[i])
					else
						--TriggerEvent('vrp_frio:verificarIsqueiro',true,coords.x,coords.y,coords.z,modelFirePit[i])
					end
				end
			end
		end	
...
if Config["debug"] then
	RegisterCommand("sc_bench", function()
		...
		Citizen.CreateThread(function()
			local bench = GetHashKey("prop_washer_03")
			RequestModel(bench)
			...
			for k,v in ipairs(Config["craft_table"]) do
				if v.position ~= nil then
					if v.ent == nil then
						if ESX then
							...
							if v.map_blip then
								...
								AddTextComponentString(v.blip_name or "Mesa de Creacciones")
					...
					if v.ent and type(v.ent) == "number" and craft_percent ~= -1 then
						draw.Simple3DText(v.position["x"], v.position["y"], v.position["z"] + 1.4, "CREANDO", 1.0)
					...
					if dist <= (v.max_distance or 2.0) and craft_percent == -1 and not menu_active then
						...
						AddTextComponentString(" ~INPUT_CONTEXT~ ~b~CRONSTRUCIONES")
