...
TriggerEvent('es:addGroupCommand', 'sqlban', Config.Permission, function (source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1DIAMANTE ', 'Permisos insuficientes.' } })
end, {help = Text.ban, params = {{name = "id"}, {name = "dias", help = Text.dayhelp}, {name = "razón", help = Text.reason}}})

TriggerEvent('es:addGroupCommand', 'sqlunban', Config.Permission, function (source, args, user)
	cmdunban(source, args)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1DIAMANTE ', 'Permisos insuficientes.' } })
end, {help = Text.unban, params = {{name = "nombre", help = Text.steamname}}})

TriggerEvent('es:addGroupCommand', 'sqlsearch', Config.Permission, function (source, args, user)
	cmdsearch(source, args)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1DIAMANTE ', 'Permisos insuficientes.' } })
end, {help = Text.bansearch, params = {{name = "nombre", help = Text.steamname}}})

TriggerEvent('es:addGroupCommand', 'sqlbanoffline', Config.Permission, function (source, args, user)
	cmdbanoffline(source, args)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1DIAMANTE ', 'Permisos insuficientes.' } })
end, {help = Text.banoff, params = {{name = "permid", help = Text.permid}, {name = "dia", help = Text.dayhelp}, {name = "razon", help = Text.reason}}})
	...
	if target and target > 0 then
		...
		if ping and ping > 0 then
			if duree and duree < 365 then
				...
				print("BanSql Error: el tiempo de Auto-Cheat-Ban no es válido.")
			end	
		else
			print("BanSql Error: el objetivo de Auto-Cheat-Ban no está en línea.")
		end
	else
		print("BanSql Error :Auto-Cheat-Ban ha recibido una identificación no válida.")
	end
end)
