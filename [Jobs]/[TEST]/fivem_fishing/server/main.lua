...
AddEventHandler('fivem_fishing:caughtFish', function(fish, weight)
			...
			if v['weight'] < weight then
				MySQL.Async.fetchAll('SELECT firstname, lastname FROM users WHERE identifier = @charid', {['@charid'] = Player['identifier']}, function(charinfo)
					...
					MySQL.Async.execute('UPDATE fivem_fishing SET data = @data WHERE fish = @fish',{
						...
						TriggerClientEvent('esx:showNotification', src, 'Votre poisson '..fish['label']..' a pris les devants à '..weight..'grammes')
					end)
				end)
			end
		end
	...
	if not found then 
		MySQL.Async.fetchAll('SELECT firstname, lastname FROM users WHERE identifier = @charid', {['@charid'] = Player['identifier']}, function(charinfo)
			...
			MySQL.Async.execute('UPDATE fivem_fishing SET data = @data WHERE fish = @fish',{
				...
				TriggerClientEvent('esx:showNotification', src, 'tu pez '..fish['label']..' tomó la delantera en'..weight..'gramos')
			end)
		end)
	end
	if item.limit > item.count then
		...
		TriggerClientEvent('esx:showNotification', src, 'Ya no puedes llevar pescado. tiralo al agua')
	end
end)
...
AddEventHandler('strandatt_fishing:sellFish', function()
	...
	if foundFish then
		...
		TriggerClientEvent('esx:showNotification', src, 'Has vendido todo tu pescado por '..cash..'$')
	else
		TriggerClientEvent('esx:showNotification', src, 'No tienes pescado para vender')
	end
end)