				...
				if distance < 15.0 then
					...
					if distance < 1.0 then
						if v['position'] == 'sell' then
							DrawText3D(v['x'], v['y'], v['z'], '[E] Hablar con el comprador')
							if IsControlJustReleased(0, 38) then
								OpenSellMenu()
							end	
						else
							DrawText3D(v['x'], v['y'], v['z'], ' [E] para Pescar')
							if IsControlJustReleased(0, 38) then
								ESX.TriggerServerCallback('fivem_fishing:checkIfPlayerCanFish', function(state)
									if state == true then
										StartFishing(k, v)
									else 
										ESX.ShowNotification("Necesitas una caña de pescar y un cebo.")
									end
								end)
								Citizen.Wait(10)
							end	
...
StartFishing = function(location, data)
	...
	while fishing do
		...
		elseif catchPeriod > 0 and randomWait <= 0 then
			...
			x = 0.85
			msg = ' [ENTER] Atrapar al Pez'
			DrawTxt(0.9, 0.75, 1.0, 1.0, 0.5, 'La tanza se Rompio', 0, 151, 226, 255)
		elseif failure > 0 then
			...
			if failure == 1 then
				...
				if breakChance == 1 then
					x = 0.825
					msg = 'El pez se escapó y rompió tu caña de pescar....'
...
OpenSellMenu = function()
	...
	local elements = {
		{['label'] = 'Vender pescado', ['value'] = 'sell_fish'}
	}
	if Config.EnableMeatSell then
		table.insert(elements, {['label'] = 'Vendre de la viande', ['value'] = 'sell_meat'})
	end
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'invite_player',
	{
		title    = 'Que te gustaria vender',
		align    = 'top-right',
		elements = elements
	}, function(data, menu)
...
GetFishProbability = function(fishes)
	...
	for name, fish in pairs(fishes) do
		...
		if rNbr <= baseVal then
			...
			ESX.ShowNotification('atrapaste un ' .. fish['label']..' à '..weight..' gramos')
		...
		elseif turn == 6 then
			...
			ESX.ShowNotification('atrapaste un ' .. defaultFish['label']..' à '..weight..' gramos')
