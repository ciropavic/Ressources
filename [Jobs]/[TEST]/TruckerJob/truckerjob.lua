...
function Chat(t)
	TriggerEvent("chatMessage", 'CAMIONEROS', { 0, 255, 255}, "" .. tostring(t))
end

--locations
--arrays
local TruckingCompany = {}
TruckingCompany[0] = {["x"] = -905.86,["y"] = 1662.37, ["z"] = 5.11}
local TruckingTrailer = {}
TruckingTrailer[0] = {["x"] = -924.59, ["y"] =1665.46, ["z"] = 4.29, ["h"] = 326.29}

local Truck = {"HAULER", "PACKER", "PHANTOM"}
local Trailer = {"TANKER", "TRAILERS", "TRAILERS2", "TRAILERLOGS"}

local MissionData = {
    [0] = {-1385.9, 269.06, 2.84, 300}, --x,y,z,money
    [1] = {-529.18, 1120.66, 4.66, 400},
    [2] = {590.64,1112.14, 8.74, 500},
    [3] = {1331.68, 2323.17, 16.77, 750},
    [4] = {2066.66, 1461.3, 18.03, 1000},
    [5] = {2190.56, 999.04, 5.81, 1500}
}
local MISSION = {}
MISSION.start = false
...
function tick()
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Conduce al ~g~destino~w~.", 10000)
            text1 = true
        elseif( not IsEntityAttached(MISSION.trailer) and text2 == false ) then
            TriggerEvent("mt:missiontext", "Ata el ~o~trailer~w~.", 15000)
            text2 = true
        end
        Wait(2000)
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "Ganas $"..currentMission[4], 5000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "Llegado. separa tu ~o~trailer~w~ con ~r~H~w~", 15000)
        end
...
function MISSION.spawnTruck()
    ...
    MISSION.truck = CreateVehicle(MISSION.hashTruck, -919.89, 1675.13, 4.29, 326.0, true, false)
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "¿Quieres ser camionero? Presiona ~r~N+~w~ para comenzar.", 500)
...
function GUI.init()
    GUI.loaded = true
    GUI.addTitle("Eres camionero ahora.", 0.425, 0.19, 0.45, 0.07 )
    GUI.addDesc("Elige un remolque.", 0.575, 0.375, 0.15, 0.30 )
    ...
    GUI.addButton(0, "Salir", GUI.exit, 0.35, 0.50, 0.3, 0.05 )
    
    GUI.buttonCount = 0
    
    GUI.addButton(1, "Mision 1 [ 300$ ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "Mision 2 [ 400$ ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "Mision 3 [ 500$ ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "Mision 4 [ 750$ ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "Mision 5 [ 1000$ ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(1, "TEST! [ 1.500$ ]", GUI.mission, 0.35, 0.50, 0.3, 0.05)
    GUI.addButton(1, "Salir Menu", GUI.exit, 0.35, 0.55, 0.3, 0.05)
end
...
function GUI.updateSelectionMenu(menu)
    ...
    elseif( IsControlPressed(0, Keys["ENTER"]) ) then
        if( type(GUI.button[menu][GUI.selected[menu]]["func"]) == "function" ) then
            ...
            Citizen.Trace("\n ¡No se puede llamar a la función! - Menú seleccionado: "..GUI.selected[menu].." \n")
        end
        GUI.time = 0
    end
    local i = 0
