...
ESX.RegisterUsableItem('turtlebait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Colocas el cebo de tortuga en tu caña de pescar")
	else
...
ESX.RegisterUsableItem('bait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Colocas el cebo de pescado en tu caña de pescar")
...
ESX.RegisterUsableItem('turtle', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message', _source, "~g~Colocas la carne de tortuga en la caña de pescar.")
	else
		TriggerClientEvent('fishing:message', _source, "~r~No tienes caña de pescar")
	end
...
ESX.RegisterUsableItem('aaaa', function(source)
...
AddEventHandler('fishing:catch', function(bait)
	...
	if bait == "turtle" then
		if rnd >= 78 then
			...
			if rnd >= 75 then
				if xPlayer.getInventoryItem('fish').count > 100 then
					TriggerClientEvent('fishing:message', _source, "~r~No puedes sostener más peces")
				else
		...
		if bait == "none" then
			...
			if rnd >= 70 then
			TriggerClientEvent('fishing:message', _source, "~y~Actualmente estás pescando sin ningún cebo equipado")
				if  xPlayer.getInventoryItem('fish').count > 100 then
						TriggerClientEvent('fishing:message', _source, "~r~No puedes sostener más peces")
					else
			...
			if rnd >= 82 then
						...
						if rnd >= 91 then
							...
							if xPlayer.getInventoryItem('shark').count > 0  then
									...
									TriggerClientEvent('fishing:message', _source, "~g~¡Atrapaste un tiburón! \n ~r~ Estas son especies en peligro de extinción y su posesión es ilegal")
	...
	local xPlayer  = ESX.GetPlayerFromId(_source)
			if item == "fish" then
					local FishQuantity = xPlayer.getInventoryItem('fish').count
						...
						TriggerClientEvent('esx:showNotification', source, '~r~No tienes suficiente~s~ Peces')			
					else   
			...
			if item == "turtle" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('esx:showNotification', source, '~r~No tienes suficiente~s~ Caparazones')			
				else   
					xPlayer.removeInventoryItem('turtle', 1)
			...
			if item == "shark" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('esx:showNotification', source, '~r~No tienes suficiente~s~ Colmillos')			
				else   
