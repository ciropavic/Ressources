...
local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
			...
			AddTextComponentString("Pescaderia")
...
local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Venta de Caparazones(Tortugas)")
...
local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Venta Colmillos(Tiburones)")
...
for _, info in pairs(Config.MarkerZones) do
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Parking Portuario")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Parar Pesca")
			end
			if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~El pez Escapo")
				end
			end
		end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~No se puede pescar conduciendo")
	else
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Presiona E Alquilar Bote')
...
function OpenBoatsMenu(x, y , z)
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un barco por', {255,0,255}, '$' .. 2500)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un barco por', {255,0,255}, '$' .. 5500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un barco por', {255,0,255}, '$' .. 7500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un barco por', {255,0,255}, '$' .. 4500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un barco por', {255,0,255}, '$' .. 3500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Sacaste un bote')
		SetPedCoordsKeepVehicle(ped, x, y , z)
