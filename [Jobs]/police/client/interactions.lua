...
AddEventHandler('police:client:checkBank', function()
	...
	if(distance ~= -1 and distance < 7) then
		...
		TriggerEvent("notification", "Ningún jugador cerca de ti!",2)
	end
end)
...
AddEventHandler('YM:RobPlayer', function()
    exports["taskbar"]:taskBar(math.random(5000, 7000) ,"Buscando...")
    TriggerEvent('disc-inventoryhud:search')
end)
...
AddEventHandler('YM:lootdeadbody', function()
    ...
    TriggerEvent('disc-inventoryhud:loot')
end)
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            TriggerEvent('notification', "Tiempo de cárcel no válido!", 2)
        end
    else
        TriggerEvent('notification', "Ningún jugador cerca de ti!", 2)
    end
...
AddEventHandler('police:client:BillPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(price) > 0 then
            ...
            TriggerEvent('notification', "Precio no válido!", 2)
        end
    else
        TriggerEvent('notification', "Ningún jugador cerca de ti!", 2)
    end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim ( GetPlayerPed(player), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "random@arrests@busted", "idle_a", 3) or ESX.PlayerData.job.name == 'police') then
                ...
                TriggerEvent("notification", "Solo puedes esposar a los jugadores que tengan las manos en alto..",2)
            end
        else
            TriggerEvent('notification', "Ningún jugador cerca de ti!", 2)
        end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim(GetPlayerPed(player), "mp_arresting", "idle", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "mp_arrest_paired", "crook_p2_back_right", 3) or ESX.PlayerData.job.name == 'police') then
                ...
                TriggerEvent("notification", "Solo puedes quitar las esposas a los jugadores que tengan las esposas.",2)
            end
        else
            TriggerEvent('notification', "Ningún jugador cerca de ti!", 2)
        end
...
AddEventHandler('police:remmask', function()
	...
	if (distance ~= -1 and distance < 2) then
		...
		TriggerEvent("notification", "Ningún jugador cerca de ti (tal vez acercarse)!",2)
	end
end)
