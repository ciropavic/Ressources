...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys["H"]) then
            print("Rotacion: " ..GetEntityRotation(GetPlayerPed(-1)))
        end
    ...
    while true do
        ...
        for k,v in pairs(Config.Shops) do
            for i=1, #v.Cams, 1 do
                ...
                if dis < 1.2 then
                    YMDrawText3D(v.Cams[i][1], v.Cams[i][2], v.Cams[i][3], 'Presione ~r~[E]~s~ Para ver las cámaras')
                    ...
                    if dis < 3 and IsControlJustPressed(1, 38) then
                        if (job == "police") then
                            ESX.Streaming.RequestAnimDict("mp_fbi_heist", function()
                                ...
                                exports['taskbar']:taskBar(7000, "Ver cámaras")
                                TriggerEvent('YM:Cams')
                            end)
                        elseif (job ~= "police") then
                            TriggerEvent("notification",'Tu no un oficial',2)
                        end
                    end
                end
            end
...
AddEventHandler('police:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        TriggerEvent('notification', "La cámara no existe..", 2)
    end
end)
