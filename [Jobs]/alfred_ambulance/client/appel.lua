...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("ems", "~b~Solicitud de ems", "Alguien necesita un ems !\n~g~Y~w~ para tomar la llamada\n~r~X~w~ para rechazar", "CHAR_CALL911", 8)
end)
     ...
     while true do
		...
		if AppelEnAttente then
			...
			elseif IsControlJustPressed(1, 73) and AppelEnAttente then
				ESX.ShowAdvancedNotification("ems", "~b~Solicitud de ems", "Rechazaste la llamada.", "CHAR_CALL911", 8)
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			ESX.ShowAdvancedNotification("ems", "~b~Solicitud de ems", "La llamada ya ha sido atendida, lo siento.", "CHAR_CALL911", 8)
		end
     end
end)
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("ems", "~b~Solicitud de ems", "Respondiste la llamada, sigue la ruta del GPS.", "CHAR_CALL911", 8)   
	...
	-- Ajout de la route
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~g~Alguien necesita reanimación aquí ...\n~b~[E]~w~Pour la réanimar.", 4, 0.1, 0.1)
...
Citizen.CreateThread( function()	
	while true do
		...
		if enService then
			...
			DrawAdvancedText(0.984, 0.214, 0.008, 0.0028, 0.4, "Última llamada recibida:", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.988, 0.236, 0.005, 0.0028, 0.4, "~b~"..NomAppel.." ~w~tomó la última llamada de EMS", 255, 255, 255, 255, 6, 0)
			DrawAdvancedText(0.984, 0.274, 0.008, 0.0028, 0.4, "Llamada total tomada en cuenta", 0, 191, 255, 255, 6, 0)
