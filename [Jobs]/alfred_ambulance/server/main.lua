...
TriggerEvent('es:addGroupCommand', 'heal', 'mod', function(source, args, user)
	if args[1] ~= nil then
		...
		TriggerClientEvent('esx_ambulancejob:heal', source)
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, { help = _U('heal_help'), params = {{ name = 'id' }} })
...
AddEventHandler('AnnounceEMSOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Anuncio EMS', 'Un EMS esta en servicio ! Tu salud ante todo !', 'CHAR_CALL911', 8)
	end
end)
...
AddEventHandler('AnnounceEMSFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~Anuncio EMS', 'Un EMS sale de servicio !', 'CHAR_CALL911', 8)
	end
end)
