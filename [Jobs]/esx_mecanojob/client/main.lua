...
function OpenMecanojobActionsMenu()
...
function OpenMecanojobHarvestMenu()
...
function OpenMecanojobCraftMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
...
function OpenMobileMecanojobActionsMenu()
	...
	elseif data.current.value == 'dep_vehicle' then
		...
		if isVehicleTow then
			...
			ESX.ShowNotification(_U('imp_flatbed3'))
		end
...
AddEventHandler('esx_mecanojob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'MecanojobActions' then
		CurrentAction     = 'mecano_actions_menu'
		CurrentActionMsg  = _U('open_actions')
		CurrentActionData = {}
...
-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.MecanojobActions.Pos.x, Config.Zones.MecanojobActions.Pos.y, Config.Zones.MecanojobActions.Pos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'mecano_harvest_menu' then
					OpenMecanojobHarvestMenu()
				elseif CurrentAction == 'mecano_craft_menu' then
					OpenMecanojobCraftMenu()
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			OpenMobileMecanojobActionsMenu()
		end
		...
		if IsControlJustReleased(0, Keys['DELETE']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
				...
				if IsPedInAnyVehicle(playerPed, false) and IsVehicleModel(GetVehiclePedIsIn(playerPed, false), GetHashKey('flatbed3')) then
					...
					ESX.ShowNotification(_U('must_in_flatbed3'))
				end
			end
		end
