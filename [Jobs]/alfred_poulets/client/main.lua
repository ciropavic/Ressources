...
-------------------------
------------------CONFIG----------------------
local startX = 2302.22  --miejsce lapania kurczakow
local startY = 4949.26
local startZ = 41.47
---------------------------------------------
...
local lapaniek = AddBlipForCoord(startX, startY, startZ)
    ...
    AddTextComponentString('Atrapa pollos')
...
local rzeznia = AddBlipForCoord(przetworniaX, przetworniaY, przetworniaZ)
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString('matadero')
...
local skupk = AddBlipForCoord(sellX, sellY, sellZ)
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString('Compra de pollos')
...
---
			...
			if dist <= 2.5 then
			DrawText3D(startX, startY, startZ, "~g~~h~[E]~h~~w~ Para empezar a cojer pollos")
			end
--
            if dist <= 0.5 then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
			...
			if dist <= 2.5 then
			DrawText3D(przetworniaX, przetworniaY, przetworniaZ, "~g~~h~[E]~h~~w~ Para porcionar el pollo")
			end
			...
			if dist2 <= 2.5 then
			DrawText3D(przetworniaX2, przetworniaY2, przetworniaZ2, "~g~~h~[E]~h~~w~ Para porcionar el pollo")
			end
			...
			DrawText3D(pakowanieX, pakowanieY, pakowanieZ, "~g~~h~[E]~h~~w~ Para envolver el pollo")
			elseif distP <= 2.5 and pakuje == 1 then
			DrawText3D(pakowanieX, pakowanieY, pakowanieZ, "~g~~h~[G]~h~~w~Para dejar de empacar")
			DrawText3D(pakowanieX, pakowanieY, pakowanieZ+0.1, "~g~~h~[E]~h~~w~ Seguir empacando")
			end
			...
			DrawText3D(pakowanieX2, pakowanieY2, pakowanieZ2, "~g~[E]~w~ Para envolver el pollo")
			elseif distP2 <= 2.5 and pakuje == 1 then
			DrawText3D(pakowanieX2, pakowanieY2, pakowanieZ2, "~g~[G]~w~ Para dejar de empacar")
			DrawText3D(pakowanieX2, pakowanieY2, pakowanieZ2+0.1, "~g~~h~[E]~h~~w~ Seguir empacando")
			end
...
LoadDict('anim@heists@box_carry@')
	...
	ESX.ShowNotification("~y~Pones pollos en el auto.")
...
if(count > 0) then
	...
	ESX.ShowNotification("~y~Sigue empacando el pollo o ve al vehículo.")
	ClearPedTasks(GetPlayerPed(-1))
	DeleteEntity(karton)
	DeleteEntity(mieso)
else
ESX.ShowNotification("~y~No tienes nada que llevarte.")
end
end
------
...
Citizen.Wait(5000)
ESX.ShowNotification("~g~Cortas un pollo.")
FreezeEntityPosition(GetPlayerPed(-1),false)
...
ESX.ShowNotification("~y~No tienes pollos.")
end
end
...
LoadDict('anim@heists@box_carry@')
	...
	if DoesEntityExist(vehicle) then
	dajdoAuta = false
	ESX.ShowNotification("~y~Pones gallinas en el auto.")
	LoadDict('anim@heists@narcotics@trash')
	TaskPlayAnim(GetPlayerPed(-1), 'anim@heists@narcotics@trash', "throw_a", 3.0, -8, -1, 63, 0, 0, 0, 0 )
	Citizen.Wait(900)
	ClearPedTasks(GetPlayerPed(-1))
...
DoScreenFadeOut(500)
Citizen.Wait(500)
SetEntityCoordsNoOffset(GetPlayerPed(-1), 2304.02, 4947.12, 41.47, 0, 0, 1)
RequestModel(GetHashKey('a_c_hen'))
while not HasModelLoaded(GetHashKey('a_c_hen')) do
Wait(1)
end
kurczak1 = CreatePed(26, "a_c_hen", 2301.18, 4945.14, 41.44, 268.9422, true, false)
kurczak2 = CreatePed(26, "a_c_hen", 2300.59, 4940.33, 41.42, 122.9422, true, false)
kurczak3 = CreatePed(26, "a_c_hen", 2304.19, 4342.27, 41.38, 6.211257, true, false)
...
if akcja == true then
	...
	Ilosczlapanych = 0
	akcja = false
	ESX.ShowNotification("~g~Ir con los pollos al vehiculo.")
	TepnijWyjscie()
	end
	
	if dist <= 1.0 then
	DrawText3D(kurczak1Coords.x, kurczak1Coords.y, kurczak1Coords.z+0.5, "~o~~h~[E]~h~~b~ Atrapa el pollo")
		if IsControlJustPressed(0, Keys['E']) then 
		Zlapany1 = 1
		ZostalZlapany()
		end	
	elseif dist2 <= 1.0 then
		DrawText3D(kurczak2Coords.x, kurczak2Coords.y, kurczak2Coords.z+0.5, "~o~~h~[E]~h~~b~ Atrapa el pollo")
		if IsControlJustPressed(0, Keys['E']) then 
		Zlapany2 = 1
		ZostalZlapany()
		end	
	elseif dist3 <= 1.0 then
		DrawText3D(kurczak3Coords.x, kurczak3Coords.y, kurczak3Coords.z+0.5, "~o~~h~[E]~h~~b~ Atrapa el pollo")
		if IsControlJustPressed(0, Keys['E']) then 
		Zlapany3 = 1
		ZostalZlapany()
		end	
...
ESX.ShowNotification("~g~Logramos atrapar el pollo!")
	if Zlapany1 == 1 then
	DeleteEntity(kurczak1)
	Zlapany1 = 0
...
ESX.ShowNotification("~r~El pollo se te escapo")
end
end
...
Citizen.CreateThread(function()
	...
	if dist <= 2.0 then
	DrawText3D(sellX, sellY, sellZ+0.1, "~o~[E]~b~ Vender pollos envasados")
		if IsControlJustPressed(0, Keys['E']) then 
		SellKurczaki()
		end	
	end
...
ESX.ShowNotification("~r~No tienes nada que vender")
end
end
