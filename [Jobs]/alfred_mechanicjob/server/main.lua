...
AddEventHandler('esx_mechanicjob:onNPCJobMissionCompleted', function()
	...
	local total   = math.random(120, 350);
...
AddEventHandler('AnnounceMecanoOuvert', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Mécanico', '~b~Anuncio Mécanico', '¡El mecánico acaba de abrir! Ven y personaliza tus vehículos !', 'CHAR_LS_CUSTOMS', 8)
	end
end)
...
AddEventHandler('AnnounceMecanoFerme', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Mécanico', '~b~Anuncio Mécanico', 'El mecánico acaba de cerrar, ven después!', 'CHAR_LS_CUSTOMS', 8)
	end
end)
...
AddEventHandler('mecano:fourriere', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local argentEntreprise = math.random(150, 300)
	local argentPerso = math.random(150, 370)

	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mechanic', function(account)
		account.addMoney(argentEntreprise)
	end)
	xPlayer.addMoney(argentPerso)
	TriggerClientEvent('esx:showAdvancedNotification', _source, "Mécanico", "~b~Destrucción de vehículos", "Le hemos devuelto las piezas de repuesto: \nEmpresa: ~g~"..argentEntreprise.."$\n~w~Billetera: ~g~"..argentPerso.."$", "CHAR_LS_CUSTOMS", 8)
end)