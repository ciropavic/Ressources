		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					...
					{label = _U('towt_bed'),  value = 'hdwrecker'},
...
function OpenCloakroomMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		elements = {
			...
			{label = _U('work_wear'), value = 'mechanic_wear'},
		...
		elseif data.current.value == 'mechanic_wear' then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				if skin.sex == 0 then
					local clothesSkin = {
					['bags_1'] = 0, ['bags_2'] = 0,
		...
		CurrentActionMsg  = _U('open2_actions')
		CurrentActionData = {}
	end)
end
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = 'Mecánico abierto',     value = 'mecano_ouvert'},
			{label = 'Mecánico cerrado',     value = 'mecano_ferme'},
