...
Citizen.CreateThread(function ()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),  -98.11, 1435.89, 13.71, true) < 2 then
			DisplayHelpText("Presiona ~g~E~s~ para ver Empleos")
...
RegisterNUICallback('2', function(data, cb)
	TriggerServerEvent('esx_jk_jobs:setJobc')
  	cb('ok')
end)
...
local v1 = vector3(-235.28, -922.47, 32.5)
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "EMPIEZA UN NUEVO EMPLEO")
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_f_y_clubbar_01", -235.28, -922.47, 31.3, 347.63, false, true)
