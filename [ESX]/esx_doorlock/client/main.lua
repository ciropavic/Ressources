		...
		if dist < 1.5 then
			if Config.Doors[k].pickable then
				if Config.Doors[k].locked then
					...
					TriggerEvent('notification', 'La puerta ya esta abierta!', 2)
				end
			else
				TriggerEvent('notification', 'La puerta tiene una cerradura demasiado fuerte!', 2)
			end
		end
	end
end)
