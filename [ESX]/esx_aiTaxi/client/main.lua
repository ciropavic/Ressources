...
AddEventHandler('esx_aiTaxi:callTaxi', function(coords)
	if customer then
		ESX.ShowHelpNotification('Un taxi ya está en camino hacia ellos.')
	else
		customer = coords
		-- get best spawnpoint
			...
			if customer ~= nil then
				...
				if distanceMeTaxi <= 40 then
					if not parkingDone then
						...
						TriggerEvent('esx:showNotification', 'Tu Taxi está aquí')
					end
	...
	while true do
		...
		if customer ~= nil then
			if taxiArrived and not inCar and not onWayBack then
				ESX.ShowHelpNotification('Presiona ~INPUT_PICKUP~ ingresar')
...
function atTarget(cancel)
	...
	if cancel then
		...
		if vehicle ~= globalTaxi then
			...
			TriggerEvent('esx:showNotification', 'Ya no se puede cancelar el taxi')
			return
		end
	end
...
function drive(x, y , z, delete, status)
	if status == 'start' then
		...
		ESX.ShowHelpNotification('Un conductor está en camino hacia usted.')
	elseif status == 'end' then
		ESX.ShowHelpNotification('Gracias por tu confianza.')
	end
