...
function OpenComponentsMenu()
	local elements = {
		{label = _U("silenciador"), value = "silent"},
		{label = _U("linterna"), value = "flashlight"},
		{label = _U("enpuñadura"), value = "grip"},
...
Citizen.CreateThread(function()
    while true do
		...
		--[[if IsControlPressed(0, Keys['G']) and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'esx_weaponsaccessories') then
	        ...
	        if IsControlPressed(0, Keys['F5']) and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'esx_weaponsaccessories_skins') then
