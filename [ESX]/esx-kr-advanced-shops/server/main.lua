      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Ya tienes un mismo artículo en tu tienda, ~r~pero para ' .. data[1].price .. '. tu pones el precio ' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Quite el artículo y ponga un nuevo precio o ponga el mismo precio')
            end
        end)  
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g Compraste una tienda para $' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~No puedes pagar esta tienda')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~El dinero de su tienda se ha bloqueado debido a un robo, espere ' .. math.floor((900 - time) / 60) .. ' minutos')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Pones en $' .. amount .. 'en tu tienda')
        else
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Sacaste $' .. amount .. ' de tu tienda')
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~No puedes vender tu tienda con artículos o dinero dentro.')
            end
        end)
    end)
end)
