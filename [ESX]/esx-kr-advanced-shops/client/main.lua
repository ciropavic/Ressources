...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Tienes: $' .. data[1].money .. ' en tu empresa',    value = ''})
		table.insert(elements, {label = 'Envíos',    value = 'shipments'})
        table.insert(elements, {label = 'Poner un artículo a la venta', value = 'putitem'})
        table.insert(elements, {label = 'Sacar un artículo a la venta',    value = 'takeitem'})
        table.insert(elements, {label = 'Poner dinero en su empresa',    value = 'putmoney'})
        table.insert(elements, {label = 'Saca dinero de tu empresa',    value = 'takemoney'})
        table.insert(elements, {label = 'Cambiar el nombre de su empresa: $' .. Config.ChangeNamePrice,    value = 'changename'})
		...
		menu2.close()
	 	...
	 	elseif data.current.value == 'putmoney' then
			...
			title = '¿Cuánto le gustaría aportar?'
	  ...
	  elseif data.current.value == 'changename' then
		...
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'changename', {
		  title = '¿Cómo le gustaría nombrar su tienda?'
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = 'Obtenga todos sus envíos', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '--ENVÍOS PENDIENTES--'})
				once = false
			end
...
function OpenShipmentDelivery(id)
	...
	local elements = {}
		...
		for i=1, #Config.Items, 1 do
			table.insert(elements, {labels =  Config.Items[i].label, label =  Config.Items[i].label .. ' a $' .. Config.Items[i].price .. ' una pieza ',	value = Config.Items[i].item, price = Config.Items[i].price})
		end
...
function TakeItem(number)
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getShopItems', function(result)
	...
	for i=1, #result, 1 do
	    if result[i].count > 0 then
	    	table.insert(elements, {label = result[i].label .. ' | ' .. result[i].count ..'piezas en almacenamiento [' .. result[i].price .. ' $ por pieza', value = 'removeitem', ItemName = result[i].item})
	    end
    end
...
local name = data.current.ItemName
    ...
    if data.current.value == 'removeitem' then
        menu.close()
        ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Cuánto cuesta', {
        title = '¿Cuánto te gustaría sacar?'
        }, function(data2, menu2)
...
function PutItem(number)
  ...
  function(data, menu)
        ...
        local invcount = data.current.count
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Vender', {
			title = _U('how_much')
			}, function(data2, menu2)
			...
			if count > invcount then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'precio de venta', {
				title = _U('set_price')
				}, function(data3, menu3)
...
Citizen.CreateThread(function()
	Citizen.Wait(500)
		local blip = AddBlipForCoord(162.81,156.34,8.59)
...
function createForSaleBlips()
	if showblip then
		...
		IDBLIPS = {
			[1] = {x = 98.25,   y = -1300.38,  z = 29.26, n = 1},
			[2] = {x = -1483.55,  y = -380.22, z = 30.16, n = 2},
			[3] = {x = 0.0,  y = 0.0, z = 0.0, n = 3},
			[4] = {x = 0.0,  y = 0.0, z = 0.0, n = 4},
			[5] = {x = 0.0,  y = 0.0, z = 0.0, n = 5},
			[6] = {x = 0.0,  y = 0.0, z = 0.0, n = 6},
			[7] = {x = 0.0,  y = 0.0, z = 0.0, n = 7},
			[8] = {x = 0.0,  y = 0.0, z = 0.0, n = 8},
			[9] = {x = 0.0,  y = 0.0, z = 0.0, n = 9},
			[10] = {x = 0.0,  y = 0.0, z = 0.0, n = 10},
			[11] = {x = 0.0,  y = 0.0, z = 0.0, n = 11},
			[12] = {x = 0.0,  y = 0.0, z = 0.0, n = 12},
			[13] = {x = 0.0,  y = 0.0, z = 0.0, n = 13},
			[14] = {x = 0.0,  y = 0.0, z = 0.0, n = 14},
			[15] = {x = 0.0,  y = 0.0, z = 0.0, n = 15},
			[16] = {x = 0.0,  y = 0.0, z = 0.0, n = 16},
			[17] = {x = 0.0,  y = 0.0, z = 0.0, n = 17},
			[18] = {x = 0.0,  y = 0.0, z = 0.0, n = 18},
			[19] = {x = 0.0,  y = 0.0, z = 0.0, n = 19},
			[20] = {x = 0.0,  y = 0.0, z = 0.0, n = 20},
		}
...
function Robbery(id)
	...
	local coords1 = {
  		[1] = {x = 100.34, y = -1304.56, z = 29.26, heading = 211.49},
		[2] = {x = -1485.50, y = -378.99, z = 40.16, heading = 131.9},
		...
		[6] = {x = -2959.5, y = 387.78, z =13.74 , heading = 171.58},
		[7] = {x = 2674.36, y = 3288.0, z = 54.84, heading = 339.44},
		...
		[19] = {x = 1737.05, y = 6418.21, z = 35.24, heading = 63.0},
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
                    ...
                    local coords = {
						...
						TriggerServerEvent('esx_phone:send', "police", "Robo de tienda en el " .. result.name .. '\'s tienda', true, coords)
						TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~r~Tu tienda ~b~(" .. result.name .. ')~r~ está siendo robada', id)
						
						ESX.Game.SpawnObject(3043770296, coords, function(safe)
				...
				ESX.ShowNotification("~r~Esta tienda ya ha sido robada, espere " ..  math.floor((Config.TimeBetweenRobberies - result.time)  / 60) .. ' minutos')
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					TriggerEvent("mt:missiontext", "Romper la Caja:~r~ " .. hp/100 .. "%", 1000)
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~g~El robo en tu tienda ~b~(" .. Name .. ')~g~ No fue exitoso!', Id)
