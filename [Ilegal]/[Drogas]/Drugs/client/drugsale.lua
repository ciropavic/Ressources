		...
		repeat
			...
			if distance < 2 and CanSellToPed(ped) and canSellDrugs then
				if canSellDrugs and not InVehicle then
					ESX.ShowHelpNotification("Presiona ~g~ ~INPUT_VEH_HEADLIGHT~ ~s~ ofrecer ~r~drogas~s~")
					if IsControlJustPressed(1,74) then
						...
						exports['taskbar']:taskBar(5000, "Ofreciendo Droga")
						Citizen.Wait(5000)
						if chance == 1 then
							...
							if chance == 1 then
								...
								ESX.ShowNotification("La persona ~r~rechazado~s~ tu oferta")
							else
								ESX.ShowNotification("La persona ~r~rechazado~s~ tu oferta")	
							end
						end
