...
ESX.RegisterUsableItem('methburn', function(source)
	if not CheckedStarted(GetPlayerIdentifier(source)) then
		...
		ESX.RegisterServerCallback("esx_newDrugs:StartMissionNow",function(source,cb)
	 	...
	 	TriggerClientEvent("esx:showNotification",source,string.format("Usted puede ~y~hackear~s~ la red de nuevo en: ~b~%s minutos~s~",GetTimeForMission(GetPlayerIdentifier(source))))
  	end
end)
...
ESX.RegisterUsableItem('meth1g', function(source)
	...
	xPlayer.removeInventoryItem("meth1g",1)
	 	...
	 	TriggerClientEvent("esx:showNotification",source,string.format("Usted puede ~r~consumir~s~ otro ~y~droga~s~ en: ~b~%s segundos~s~",GetTimeForDrugs(GetPlayerIdentifier(source))))
	end
end)
...
ESX.RegisterUsableItem('cokebrick', function(source)
	...
	if not bags or not brick then
		if not bags then
			TriggerClientEvent("esx:showNotification",source,"usted ~r~no tener~s~ suficiente ~y~Bolsitas~s~")
		else
			TriggerClientEvent("esx:showNotification",source,"usted ~r~no tener~s~ suficiente ~y~Ladrillos de cocaína~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("coke10g").count <= 40 or (not scale and xPlayer.getInventoryItem("coke10g").count <= 44) then
		if not CheckedStartedConvert(GetPlayerIdentifier(source)) then
			...
			TriggerClientEvent("esx:showNotification",source,string.format("Usted está ~b~ya comprometido~s~ en un ~y~proceso~s~!",GetTimeForConvert(GetPlayerIdentifier(source))))
		end
	else
		TriggerClientEvent("esx:showNotification",source,"usted ~r~do no tengo~s~ suficiente ~b~espacio vacio~s~ para más ~y~Cocaina (10G)~s~")
	end
end)
...
ESX.RegisterUsableItem('methbrick', function(source)
	...
	if not bags or not brick then
		if not bags then
			...
			TriggerClientEvent("esx:showNotification",source,"usted ~r~no tener~s~ suficiente ~y~Ladrillo de Metha~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("meth10g").count <= 40 or (not scale and xPlayer.getInventoryItem("meth10g").count <= 44) then
		...
		TriggerClientEvent("esx:showNotification",source,"usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Metha (10G)~s~")
	end
end)
...
ESX.RegisterUsableItem('weedbrick', function(source)
	...
	if not bags or not brick then
		if not bags then
			...
			TriggerClientEvent("esx:showNotification",source,"usted ~r~no tener~s~ suficiente ~y~Ladrillo de Hierba~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("weed20g").count <= 90 or (not scale and xPlayer.getInventoryItem("weed20g").count <= 92) then
		...
		TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Hierba (20G)~s~")
	end
end)
...
ESX.RegisterUsableItem('coke10g', function(source)
	...
	if not bags or not coke then
		if not bags then
			...
			TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~y~Cocaina (10G)~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("coke1g").count <= 40 or (not scale and xPlayer.getInventoryItem("coke1g").count <= 44) then
		...
		TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Cocaina (1G)~s~")
	end
end)
...
ESX.RegisterUsableItem('meth10g', function(source)
	...
	if not bags or not meth then
		if not bags then
			...
			TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~y~Metha (10G)~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("meth1g").count <= 40 or (not scale and xPlayer.getInventoryItem("meth1g").count <= 44) then
		if not CheckedStartedConvert(GetPlayerIdentifier(source)) then
			...
			TriggerClientEvent("esx:showNotification",source,string.format("Usted esta ~b~ya comprometido~s~ en un ~y~proceso~s~!",GetTimeForConvert(GetPlayerIdentifier(source))))
		end
	else
		TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Metha (1G)~s~")
	end
end)
...
ESX.RegisterUsableItem('weed20g', function(source)
	...
	if not bags or not weedBag then
		if not bags then
			...
			TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~y~Hierba (20G)~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("weed4g").count <= 195 or (not scale and xPlayer.getInventoryItem("weed4g").count <= 196) then
		...
		TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Hierba (4G)~s~")
	end
end)
...
ESX.RegisterUsableItem('weed4g', function(source)
	...
	if not paper or not weed then
		if not paper then
			TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~y~Papel de Liar~s~")
		else
			TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~y~Hierba (4G)~s~")
		end
		return
	end
	...
	if xPlayer.getInventoryItem("joint2g").count <= 48 then
		...
		TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Porro (2G)~s~")
	end
end)

-- Cogollos (1Cogollo) >> Porro (1Porro)
ESX.RegisterUsableItem('marijuana', function(source)
		
	local xPlayer = ESX.GetPlayerFromId(source)
	local weed = xPlayer.getInventoryItem("marijuana").count >= 1
	local paper = xPlayer.getInventoryItem("rolpaper").count >= 2
	...
	if xPlayer.getInventoryItem("joint2g").count <= 48 then
		if not CheckedStartedConvert(GetPlayerIdentifier(source)) then
			...
			TriggerClientEvent("Maria4gToJoint2g",source)
			Citizen.Wait(15000)
		
			xPlayer.addInventoryItem("joint2g",1)
		else
			TriggerClientEvent("esx:showNotification",source,string.format("Usted está ~b~ya comprometido~s~ en un ~y~proceso~s~!",GetTimeForConvert(GetPlayerIdentifier(source))))
		end
	else
		TriggerClientEvent("esx:showNotification",source,"Usted ~r~no tener~s~ suficiente ~b~espacio vacio~s~ para más ~y~Porros~s~")
	end
end)
...
AddEventHandler("esx_newDrugs:sellDrugs", function()
	...
	local weedd = xPlayer.getInventoryItem('marijuana').count
	local drugamount = 0
	local price = 0
	local drugType = nil
	...
	elseif coke > 0 then
		...
		TriggerClientEvent('esx:showNotification', _source, "Tienes ~r~no más~r~ ~y~drogas~s~ en ti")
		return
	end
	...
	--xPlayer.addAccountMoney('black_money', price) --Dinero como Oficial
	xPlayer.addInventoryItem('cash', price) --Dinero como Items
	if drugType=='weed4g' then
	TriggerClientEvent('esx:showNotification', _source, "Vendiste ~b~"..drugamount.."x~s~ ~y~Hierba (4G)~s~ por ~r~$" .. price)
	elseif drugType=='marijuana' then
	TriggerClientEvent('esx:showNotification', _source, "Vendiste ~b~"..drugamount.."x~s~ ~y~Marihuana ~s~ por ~r~$" .. price)
	elseif drugType=='meth1g' then
	TriggerClientEvent('esx:showNotification', _source, "Vendiste ~b~"..drugamount.."x~s~ ~y~Metha (1G)~s~ por ~r~$" .. price)
	elseif drugType=='coke1g' then
	TriggerClientEvent('esx:showNotification', _source, "Vendiste ~b~"..drugamount.."x~s~ ~y~Cocaina (1G)~s~ por ~r~$" .. price)
	end
...
AddEventHandler("esx_newDrugs:canSellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		local soldAmount = (xPlayer.getInventoryItem("coke1g").count > 0 or xPlayer.getInventoryItem("marijuana").count > 0 or xPlayer.getInventoryItem("meth1g").count > 0 or xPlayer.getInventoryItem("weed4g").count > 0) and CheckSoldAmount(xPlayer.getIdentifier()) < Config.maxCap
...
AddEventHandler("esx_drugSale:canSellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		local sell = (xPlayer.getInventoryItem("coke1g").count > 0 or xPlayer.getInventoryItem("marijuana").count > 0 or xPlayer.getInventoryItem("meth1g").count > 0 or xPlayer.getInventoryItem("weed4g").count > 0) and CheckSellAmount(xPlayer.getIdentifier()) < 150
