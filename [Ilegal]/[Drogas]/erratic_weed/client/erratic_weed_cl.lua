...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('weed:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Wait(0) end
    ESX.TriggerServerCallback('weed:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
		...
		if not inUse then
			if dist <= 1 then
				...
				if IsControlJustPressed(1, 51) then
					ESX.TriggerServerCallback('weed:pay', function(success)
						exports['mythic_notify']:SendAlert('error', 'La avioneta te espera no olvides buscar un Bidon de Gasolina por la zona!')
						if success then
							main()
						end
					end)
...
RegisterNetEvent('weed:syncTable')
AddEventHandler('weed:syncTable', function(bool)
    inUse = bool
end)

RegisterNetEvent('weed:onUse')
AddEventHandler('weed:onUse', function(source)
	if Config.useMythic then
		exports['mythic_notify']:DoLongHudText('inform', _U'weed_use')
	end
	local crackhead = GetPlayerPed(-1)
	SetPedArmour(crackhead, 30)
	SetTimecycleModifier("DRUG_gas_huffin")
	Citizen.Wait(Config.weedTime)
	DoScreenFadeOut(1000)
	Citizen.Wait(1000)
	DoScreenFadeIn(2000)
	if Config.useMythic then
		exports['mythic_notify']:DoLongHudText('inform', _U'weed_off')
	end
...
function main()
	TriggerServerEvent('weed:updateTable', true)
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				...
				TriggerServerEvent('weed:updateTable', false)
				checkPlane = false
			end
		else
			sleep = 3000
...
function delivery()
	...
	exports['mythic_notify']:SendAlert('error', 'Entra en el avion y ve al punto marcado para recoger el material!')
...
function final()
	...
	SetBlipRoute(blip, true)
	hangar = true
	exports['mythic_notify']:SendAlert('error', 'Vuelve al hangar!')
	local player = GetPlayerPed(-1)
	Citizen.CreateThread(function()
		while hangar do
			sleep = 5	
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Citizen.Wait(2000)
					TriggerServerEvent('weed:GiveItem')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			sleep = 5
			DrawText3Ds(process.x, process.y, process.z, _U'break_weed')			
			if IsControlJustPressed(1, 51) then		
				isProcessing = true
				ESX.TriggerServerCallback('weed:process', function(success)
					if success then					
						processing()
					else
						isProcessing = false
...
function processing()
	...
	FreezeEntityPosition(player, true)
	if Config.progBar then
		exports['progressBars']:startUI(6000, _U'breaking_weed')
	end
	playAnim("anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 6000)
	Citizen.Wait(6000)
	FreezeEntityPosition(player, false)
	TriggerServerEvent('weed:processed')
	isProcessing = false
end
