...
Citizen.CreateThread(function()
	while true do
		...
		if (ESX.PlayerData.job and ESX.PlayerData.job.name == Config.marabuntaDatabaseName) then
			...
			for i = 1, #v.Pos, 1 do
				...
				if (distance < 1.0) and insideMarker == false then
					...
					if IsControlJustPressed(0, Config.KeyToOpenArmory) then
						ESX.TriggerServerCallback('esx_marabuntaArmory:getWeaponState', function(stock) end)
						marabuntaArmory()
						insideMarker = true
						Citizen.Wait(500)
					end
...
marabuntaArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_marabuntaArmory_main_menu",
		{
			title    = Config.marabuntaArmoryTitle,
			align    = "top-left",
			elements = elements
		},
	function(data, menu)
...
WeaponMenu = function()
	...
	ESX.TriggerServerCallback("esx_marabuntaArmory:getWeaponState", function(stock)	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_marabuntaArmory_weapon_storage",
		{
	...
	function(data, menu)
		...
		if data.current.lendable == true then
			...
			TriggerServerEvent("esx_marabuntaArmory:weaponTakenOut", data.current.weaponhash, giveAmmo)
		elseif PedHasWeapon(data.current.weaponhash) then
			local giveAmmo = (GetWeaponClipSize(GetHashKey(data.current.weaponhash)) > 0)
			if data.current.weaponhash == "WEAPON_STUNGUN" then
				giveAmmo = false
			end
			TriggerServerEvent("esx_marabuntaArmory:weaponInStock", data.current.weaponhash,GetAmmoInPedWeapon(ped,GetHashKey(data.current.weaponhash)),giveAmmo)
		else
...
function RestockMenu()
	local marabunta = {}
	local elements = {}
	ESX.TriggerServerCallback("esx_marabuntaArmory:checkmarabuntaOnline", function(list) marabunta = list end)
	Citizen.Wait(250)
	for k,v in pairs(marabunta) do
		if v.job.name == Config.marabuntaDatabaseName then
			table.insert(elements, {label = v.name, id = v.id})
		end
	end
	if next(elements) ~= nil then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_marabuntaArmory_restock_menu",
			{
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_marabuntaArmory:restockWeapons",data.current.id)
		end, function(data, menu)
		...
		ESX.ShowNotification(Config.NomarabuntaOnline)
	end
end
...
function KevlarMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_marabuntaArmory_kevlar_menu",
			{
				title    = Config.marabuntaKevlarTitle,
				align    = "top-left",
				elements = {
...
function AttachmentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_marabuntaArmory_attachment_menu",
		{
...
function ListOfAttachments(type,name,weaponhash,attachment,clip,scope,suppressor)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'marabuntaArmory_list_of_attachments', {
