...
Citizen.CreateThread(function()
	while true do
		...
		if (ESX.PlayerData.job and ESX.PlayerData.job.name == Config.thefamilieDatabaseName) then
			...
			for i = 1, #v.Pos, 1 do
				...
				if (distance < 1.0) and insideMarker == false then
					...
					if IsControlJustPressed(0, Config.KeyToOpenArmory) then
						ESX.TriggerServerCallback('esx_thefamilieArmory:getWeaponState', function(stock) end)
						thefamilieArmory()
						insideMarker = true
						Citizen.Wait(500)
					end
...
thefamilieArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_thefamilieArmory_main_menu",
		{
			title    = Config.thefamilieArmoryTitle,
			align    = "top-left",
			elements = elements
		},
	function(data, menu)
...
WeaponMenu = function()
	...
	ESX.TriggerServerCallback("esx_thefamilieArmory:getWeaponState", function(stock)	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_thefamilieArmory_weapon_storage",
		{
	...
	function(data, menu)
		...
		if data.current.lendable == true then
			...
			TriggerServerEvent("esx_thefamilieArmory:weaponTakenOut", data.current.weaponhash, giveAmmo)
		elseif PedHasWeapon(data.current.weaponhash) then
			local giveAmmo = (GetWeaponClipSize(GetHashKey(data.current.weaponhash)) > 0)
			if data.current.weaponhash == "WEAPON_STUNGUN" then
				giveAmmo = false
			end
			TriggerServerEvent("esx_thefamilieArmory:weaponInStock", data.current.weaponhash,GetAmmoInPedWeapon(ped,GetHashKey(data.current.weaponhash)),giveAmmo)
		else
...
function RestockMenu()
	local thefamilie = {}
	local elements = {}
	ESX.TriggerServerCallback("esx_thefamilieArmory:checkthefamilieOnline", function(list) thefamilie = list end)
	Citizen.Wait(250)
	for k,v in pairs(thefamilie) do
		if v.job.name == Config.thefamilieDatabaseName then
			table.insert(elements, {label = v.name, id = v.id})
		end
	end
	if next(elements) ~= nil then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_thefamilieArmory_restock_menu",
			{
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_thefamilieArmory:restockWeapons",data.current.id)
		end, function(data, menu)
		...
		ESX.ShowNotification(Config.NothefamilieOnline)
	end
end
...
function KevlarMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_thefamilieArmory_kevlar_menu",
			{
				title    = Config.thefamilieKevlarTitle,
				align    = "top-left",
				elements = {
...
function AttachmentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_thefamilieArmory_attachment_menu",
		{
...
function ListOfAttachments(type,name,weaponhash,attachment,clip,scope,suppressor)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'thefamilieArmory_list_of_attachments', {
