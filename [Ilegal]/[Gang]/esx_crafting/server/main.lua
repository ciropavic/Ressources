...
AddEventHandler('rs_crafting:CraftingFailed', function(CraftItem)
    ...
    if rand >= 50 then
        TriggerClientEvent('esx:showNotification', src, "Por suerte todavía tienes tus artículos..")
    else
        for itemname, v in pairs(item.needs) do
            xPlayer.removeInventoryItem(itemname, v.count)
        end
    ...
    TriggerClientEvent('esx:showNotification', src, "~r~No pudo hacer ~b~"..item.label)
end)
