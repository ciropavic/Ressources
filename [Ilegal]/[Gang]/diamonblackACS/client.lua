...
Citizen.CreateThread(function()
	while true do
		...
		if (ESX.PlayerData.job and ESX.PlayerData.job.name == Config.diamonblackDatabaseName) then
			...
			for i = 1, #v.Pos, 1 do
				...
				if (distance < 1.0) and insideMarker == false then
					...
					if IsControlJustPressed(0, Config.KeyToOpenArmory) then
						ESX.TriggerServerCallback('esx_diamonblackArmory:getWeaponState', function(stock) end)
						diamonblackArmory()
						insideMarker = true
						Citizen.Wait(500)
					end
...
diamonblackArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_diamonblackArmory_main_menu",
		{
			title    = Config.diamonblackArmoryTitle,
			align    = "top-left",
			elements = elements
		},
	function(data, menu)
...
WeaponMenu = function()
	...
	ESX.TriggerServerCallback("esx_diamonblackArmory:getWeaponState", function(stock)	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_diamonblackArmory_weapon_storage",
		{
	...
	function(data, menu)
		...
		if data.current.lendable == true then
			...
			TriggerServerEvent("esx_diamonblackArmory:weaponTakenOut", data.current.weaponhash, giveAmmo)
		elseif PedHasWeapon(data.current.weaponhash) then
			local giveAmmo = (GetWeaponClipSize(GetHashKey(data.current.weaponhash)) > 0)
			if data.current.weaponhash == "WEAPON_STUNGUN" then
				giveAmmo = false
			end
			TriggerServerEvent("esx_diamonblackArmory:weaponInStock", data.current.weaponhash,GetAmmoInPedWeapon(ped,GetHashKey(data.current.weaponhash)),giveAmmo)
		else
...
function RestockMenu()
	local diamonblack = {}
	local elements = {}
	ESX.TriggerServerCallback("esx_diamonblackArmory:checkdiamonblackOnline", function(list) diamonblack = list end)
	Citizen.Wait(250)
	for k,v in pairs(diamonblack) do
		if v.job.name == Config.diamonblackDatabaseName then
			table.insert(elements, {label = v.name, id = v.id})
		end
	end
	if next(elements) ~= nil then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_diamonblackArmory_restock_menu",
			{
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_diamonblackArmory:restockWeapons",data.current.id)
		end, function(data, menu)
		...
		ESX.ShowNotification(Config.NodiamonblackOnline)
	end
end
...
function KevlarMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_diamonblackArmory_kevlar_menu",
			{
				title    = Config.diamonblackKevlarTitle,
				align    = "top-left",
				elements = {
...
function AttachmentMenu()
	...
	for k,v in pairs(Config.WeaponsInArmory) do
		if v.attachment == true then
			table.insert(elements,{label = v.label, weaponhash = v.weaponHash, type = v.type, attachment = v.attachment, clip = v.clip, scope = v.scope, suppressor = v.suppressor})
		end
	end

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_diamonblackArmory_attachment_menu",
		{
	...
	function(data, menu)
			if data.current.weaponhash == data.current.weaponhash then
				if GetSelectedPedWeapon(ped) == GetHashKey(data.current.weaponhash) then
					ListOfAttachments(data.current.type, data.current.label, data.current.weaponhash, data.current.attachment, data.current.clip, data.current.scope, data.current.suppressor)
				else
...
function ListOfAttachments(type,name,weaponhash,attachment,clip,scope,suppressor)
	local elements = {}
	
	local ped = GetPlayerPed(-1)
			
	if clip then
		local state = HasPedGotWeaponComponent(ped, weaponhash, clip)
		local text
		
		if state then
			text = "Cargador: "..('<span style="color:green;">%s</span>'):format("On")
		else
			text = "Cargador: "..('<span style="color:red;">%s</span>'):format("Off")
		end
	...
	if scope then
		...
		if state then
			text = "Alcance: "..('<span style="color:green;">%s</span>'):format("On")
		else
			text = "Alcance: "..('<span style="color:red;">%s</span>'):format("Off")
		end
	...
	if suppressor then
		...
		if state then
			text = "Supresor: "..('<span style="color:green;">%s</span>'):format("On")
		else
			text = "Supresor: "..('<span style="color:red;">%s</span>'):format("Off")
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'diamonblackArmory_list_of_attachments', {
		...
		if data.current.value == clip then
			if data.current.state then
				newData.label = "Cargador: "..('<span style="color:green;">%s</span>'):format("On")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.clipEquipped,name))
			else
				newData.label = "Cargador: "..('<span style="color:red;">%s</span>'):format("Off")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.clipRemove,name))
			end
		elseif data.current.value == scope then
			if data.current.state then
				newData.label = "Alcance: "..('<span style="color:green;">%s</span>'):format("On")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeEquipped,name))
			else
				newData.label = "Alcance: "..('<span style="color:red;">%s</span>'):format("Off")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeRemove,name))
			end
		elseif data.current.value == suppressor then
			if data.current.state then
				newData.label = "Supresor: "..('<span style="color:green;">%s</span>'):format("On")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.SuppressorEquipped,name))
			else
				newData.label = "Supresor: "..('<span style="color:red;">%s</span>'):format("Off")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
