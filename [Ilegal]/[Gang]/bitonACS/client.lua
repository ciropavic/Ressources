...
Citizen.CreateThread(function()
	while true do
		...
		if (ESX.PlayerData.job and ESX.PlayerData.job.name == Config.bitonDatabaseName) then
			...
			for i = 1, #v.Pos, 1 do
				...
				if (distance < 1.0) and insideMarker == false then
					...
					if IsControlJustPressed(0, Config.KeyToOpenArmory) then
						ESX.TriggerServerCallback('esx_bitonArmory:getWeaponState', function(stock) end)
						bitonArmory()
						insideMarker = true
						Citizen.Wait(500)
					end
...
bitonArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bitonArmory_main_menu",
		{
			title    = Config.bitonArmoryTitle,
			align    = "top-left",
			elements = elements
		},
	function(data, menu)
...
WeaponMenu = function()
	...
	ESX.TriggerServerCallback("esx_bitonArmory:getWeaponState", function(stock)	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bitonArmory_weapon_storage",
		{
	...
	function(data, menu)
		...
		if data.current.lendable == true then
			...
			TriggerServerEvent("esx_bitonArmory:weaponTakenOut", data.current.weaponhash, giveAmmo)
		elseif PedHasWeapon(data.current.weaponhash) then
			local giveAmmo = (GetWeaponClipSize(GetHashKey(data.current.weaponhash)) > 0)
			if data.current.weaponhash == "WEAPON_STUNGUN" then
				giveAmmo = false
			end
			TriggerServerEvent("esx_bitonArmory:weaponInStock", data.current.weaponhash,GetAmmoInPedWeapon(ped,GetHashKey(data.current.weaponhash)),giveAmmo)
		else
...
function RestockMenu()
	local biton = {}
	local elements = {}
	ESX.TriggerServerCallback("esx_bitonArmory:checkbitonOnline", function(list) biton = list end)
	Citizen.Wait(250)
	for k,v in pairs(biton) do
		if v.job.name == Config.bitonDatabaseName then
			table.insert(elements, {label = v.name, id = v.id})
		end
	end
	if next(elements) ~= nil then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bitonArmory_restock_menu",
			{
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_bitonArmory:restockWeapons",data.current.id)
		end, function(data, menu)
		...
		ESX.ShowNotification(Config.NobitonOnline)
	end
end
...
function KevlarMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bitonArmory_kevlar_menu",
			{
				title    = Config.bitonKevlarTitle,
				align    = "top-left",
				elements = {
...
function AttachmentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bitonArmory_attachment_menu",
		{
...
function ListOfAttachments(type,name,weaponhash,attachment,clip,scope,suppressor)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bitonArmory_list_of_attachments', {
