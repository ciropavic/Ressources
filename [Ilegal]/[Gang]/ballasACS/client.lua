			...
			for i = 1, #v.Pos, 1 do
				...
				if (distance < 1.0) and insideMarker == false then
					...
					if IsControlJustPressed(0, Config.KeyToOpenArmory) then
						ESX.TriggerServerCallback('esx_ballasArmory:getWeaponState', function(stock) end)
						ballasArmory()
						insideMarker = true
						Citizen.Wait(500)
					end
...
ballasArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_ballasArmory_main_menu",
		{
			title    = Config.ballasArmoryTitle,
			align    = "top-left",
			elements = elements
		},
	function(data, menu)
...
WeaponMenu = function()
	...
	ESX.TriggerServerCallback("esx_ballasArmory:getWeaponState", function(stock)	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_ballasArmory_weapon_storage",
		{
	...
	function(data, menu)
		...
		if data.current.lendable == true then
			...
			TriggerServerEvent("esx_ballasArmory:weaponTakenOut", data.current.weaponhash, giveAmmo)
		elseif PedHasWeapon(data.current.weaponhash) then
			local giveAmmo = (GetWeaponClipSize(GetHashKey(data.current.weaponhash)) > 0)
			if data.current.weaponhash == "WEAPON_STUNGUN" then
				giveAmmo = false
			end
			TriggerServerEvent("esx_ballasArmory:weaponInStock", data.current.weaponhash,GetAmmoInPedWeapon(ped,GetHashKey(data.current.weaponhash)),giveAmmo)
		else
...
function RestockMenu()
	local ballas = {}
	local elements = {}
	ESX.TriggerServerCallback("esx_ballasArmory:checkballasOnline", function(list) ballas = list end)
	Citizen.Wait(250)
	for k,v in pairs(ballas) do
		if v.job.name == Config.ballasDatabaseName then
			table.insert(elements, {label = v.name, id = v.id})
		end
	end
	if next(elements) ~= nil then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_ballasArmory_restock_menu",
			{
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_ballasArmory:restockWeapons",data.current.id)
		end, function(data, menu)
		...
		ESX.ShowNotification(Config.NoballasOnline)
	end
end
...
function KevlarMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_ballasArmory_kevlar_menu",
			{
				title    = Config.ballasKevlarTitle,
				align    = "top-left",
				elements = {
...
function AttachmentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_ballasArmory_attachment_menu",
		{
...
function ListOfAttachments(type,name,weaponhash,attachment,clip,scope,suppressor)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ballasArmory_list_of_attachments', {
