...
Citizen.CreateThread(function()
	while true do
		...
		if (ESX.PlayerData.job and ESX.PlayerData.job.name == Config.bloodDatabaseName) then
			...
			for i = 1, #v.Pos, 1 do
				...
				if (distance < 1.0) and insideMarker == false then
					...
					if IsControlJustPressed(0, Config.KeyToOpenArmory) then
						ESX.TriggerServerCallback('esx_bloodArmory:getWeaponState', function(stock) end)
						bloodArmory()
						insideMarker = true
						Citizen.Wait(500)
					end
...
bloodArmory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bloodArmory_main_menu",
		{
			title    = Config.bloodArmoryTitle,
			align    = "top-left",
			elements = elements
		},
	function(data, menu)
...
WeaponMenu = function()
	...
	ESX.TriggerServerCallback("esx_bloodArmory:getWeaponState", function(stock)	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bloodArmory_weapon_storage",
		{
	...
	function(data, menu)
		...
		if data.current.lendable == true then
			...
			TriggerServerEvent("esx_bloodArmory:weaponTakenOut", data.current.weaponhash, giveAmmo)
		elseif PedHasWeapon(data.current.weaponhash) then
			local giveAmmo = (GetWeaponClipSize(GetHashKey(data.current.weaponhash)) > 0)
			if data.current.weaponhash == "WEAPON_STUNGUN" then
				giveAmmo = false
			end
			TriggerServerEvent("esx_bloodArmory:weaponInStock", data.current.weaponhash,GetAmmoInPedWeapon(ped,GetHashKey(data.current.weaponhash)),giveAmmo)
		else
...
function RestockMenu()
	local blood = {}
	local elements = {}
	ESX.TriggerServerCallback("esx_bloodArmory:checkbloodOnline", function(list) blood = list end)
	Citizen.Wait(250)
	for k,v in pairs(blood) do
		if v.job.name == Config.bloodDatabaseName then
			table.insert(elements, {label = v.name, id = v.id})
		end
	end
	if next(elements) ~= nil then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bloodArmory_restock_menu",
			{
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_bloodArmory:restockWeapons",data.current.id)
		end, function(data, menu)
		...
		ESX.ShowNotification(Config.NobloodOnline)
	end
end
...
function KevlarMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bloodArmory_kevlar_menu",
			{
				title    = Config.bloodKevlarTitle,
				align    = "top-left",
				elements = {
...
function AttachmentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "esx_bloodArmory_attachment_menu",
		{
...
function ListOfAttachments(type,name,weaponhash,attachment,clip,scope,suppressor)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bloodArmory_list_of_attachments', {
