	...
	while true do
		...
		local v = Config.ReceleurLocation
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Presione ~INPUT_CONTEXT~ para hablar con el ~y~Receptor del vehículo")
			...
			if event_is_running then
				...
				if pVehicle == event_vehicle1 or event_vehicle2 or event_vehicle3 then
					...
					if delivery_point_distance < 50.0 then
					 ...
					 DrawMissionText("Vuelve al vehículo!", 1000)
				end
			end
	end
end)
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if GetVehicleEngineHealth(event_vehicle1) < 40 and event_vehicle1 ~= nil then
							ResetVehRun()
							police_alerted = false
							DisplayMissionFailed('El vehículo está demasiado dañado')
						end
						if GetEntityHealth(event_vehicle1) == 0 and event_vehicle1 ~= nil then
							ResetVehRun()
							police_alerted = false
							DisplayMissionFailed('El vehículo estalló')
						end
						if IsEntityInWater(event_vehicle1) and event_vehicle1 ~= nil then
							ResetVehRun()
							police_alerted = false
							DisplayMissionFailed('El vehículo se hundió')
						end
						...
						if event_time_passed > 600 then
							ResetVehRun()
							police_alerted = false
							DisplayMissionFailed('El tiempo de entrega ha expirado')
						end
...
function VehRunMenu()
	ClearMenu()
    options.menu_title = "Receptor de vehículo"
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Entrega de nivel " .. Config.Scenarios[i].vehlv .. "~r~ + $" .. Config.Scenarios[i].VehRunReward, "PurchaseVehRun", i)
   	end
    Menu.addButton("Salir","CloseMenu",nil)
end
...
function AlertThePolice()
    ...
    TriggerServerEvent('esx_addons_gcphone:startCallAnonyme', 'police', "Atención ! Un vehículo sospechoso con matrícula. [".. vehicle_plate .."] se dirige al norte", PlayerCoords, {
    ...
    if PlayerData.job.name ~= Config.AlertExtraSocieties[i] then
    	...
    	TriggerServerEvent('esx_addons_gcphone:startCallAnonyme', Config.AlertExtraSocieties[i], "Atención ! Un vehículo sospechoso con matrícula. [".. vehicle_plate .."] se dirige al norte", PlayerCoords, {
			PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
		})
...
function PurchaseVehRun(scenario)
	...
	if event_is_running == true then
		...
		exports.pNotify:SendNotification({
            text = "Ya estás en una misión de entrega.",
            type = "error",
            timeout = 10000,
            layout = "centerRight"
        })
	...
	print("Policía en la ciudad (mínimo): " .. Config.Scenarios[scenario].MinCopsOnline .. " ||  Dificultad:  " .. Config.Scenarios[scenario].vehlv .. "")
	...
	ESX.TriggerServerCallback('mg_live:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				exports.pNotify:SendNotification({
            text = "Vehículo recuperado con <b style='color:green'>exito</b>.",
            type = "success",
            timeout = 10000,
            layout = "centerRight"
        })
				...
				AddTextComponentString('Punto dde Entrega')
				...
				SetBlipRoute(event_delivery_blip, 1)
						...
						if Config.Scenarios[scenario].vehlv == 3 then
							...
							exports.pNotify:SendNotification({
					text = "Este vehículo está equipado con <b style='color:blue'>kit NOS</b> <b style='color:red'>instable</b>.</br>Realice la entrega antes del <b style='color:blue'>NOS</b> <b style='color:red'>explose</b>.</br>Seguir adelante [E] usar el NOS",
					type = "info",
					timeout = 15000,
					layout = "centerRight",
					queue = "left"
						...
						elseif Config.Scenarios[scenario].vehlv ~= 3 then
							...
							exports.pNotify:SendNotification({
					text = "No te quedes !</br>El cliente espera esta entrega en 10 minutos.",
					type = "info",
					timeout = 15000,
					layout = "centerRight",
					queue = "left"
			...
			exports.pNotify:SendNotification({
            text = "Necesitas al menos ~b~" .. Config.Scenarios[scenario].MinCopsOnline .. " policías ~w~en ciudad.",
            type = "error",
            timeout = 10000,
            layout = "centerRight"
        })
