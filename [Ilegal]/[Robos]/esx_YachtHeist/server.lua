...
AddEventHandler('esx_YachtHeist:safeRobbed', function(id, state)
	...
	TriggerClientEvent("esx:showNotification",source,"conseguiste: ~r~$"..cashReward.."~s~ dinero y ~b~"..itemReward.."x~s~ ~y~Gramos de Cocaina~s~")
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:updateCashTaken",function(source,cb)
	...
	if xPlayer then
		local randomMoney = 0
		if Config.RewardBasedOnCops then
			randomMoney = ((math.random(Config.MinCashTake, Config.MaxCashTake) * 10) * policeReward)
		else
			randomMoney = (math.random(Config.MinCashTake, Config.MaxCashTake) * 50)
		end
...
ESX.RegisterServerCallback("esx_YachtHeist:getHackerDevice",function(source,cb)
	...
	if xPlayer.getInventoryItem(Config.HackerDevice).count >= 1 then
		...
		cb(false)
		TriggerClientEvent("esx:showNotification",source,"Nesesitas ~y~Hacker Dispositivo~s~ para hackear el terminal!")
	end
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:getDrillItem",function(source,cb)
	...
	if xPlayer.getInventoryItem(Config.DrillItem).count >= 1 then
		...
		cb(false)
		TriggerClientEvent("esx:showNotification",source,"Tu necesitas un ~y~Taladro~s~ para taladrar!")
	end
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:getCooldownHeist",function(source,cb)
	...
	if YachtHeist[1].recentlyRobbed == false then
		...
		cb(false)
		TriggerClientEvent("esx:showNotification",source,string.format("Nuevo atraco disponible en: ~b~%s minutos~s~",GetCooldownTimer()))
	end
end)
