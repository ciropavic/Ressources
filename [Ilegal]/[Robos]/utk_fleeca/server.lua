...
Doors = {
    ["F1"] = {{loc = vector3(966.48, 1416.96, 22.25), h = 60.91, txtloc = vector3(966.48, 1416.96, 22.25), obj = nil, locked = true}, {loc = vector3(966.48, 1416.96, 22.25), txtloc = vector3(966.48, 1416.96, 22.25), state = nil, locked = true}},
        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Este banco fue robado recientemente. Necesitas esperar "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Este banco está siendo robado actualmente.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "No tienes una tarjeta de acceso maliciosa.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "No hay suficiente policía en la ciudad..")
    end
end)
